...
func difFFT(a []fr.Element, twiddles [][]fr.Element, stage, maxSplits int, chDone chan struct{}) {
	...
	// we fire runtime.NumCPU workers for the first part (ie where we just parallelize the butterflys)
	type bWork struct {
		wg    *sync.WaitGroup
		b     []fr.Element
		m     int
		...
		go func() {
			...
			for w := range chWork {
				for i := w.start; i < w.end; i++ {
					t = w.b[i]
					w.b[i].Add(&w.b[i], &w.b[i+w.m])
					w.b[i+w.m].
						Sub(&t, &w.b[i+w.m]).
						Mul(&w.b[i+w.m], &twiddles[w.stage][i])
				}
				w.wg.Done()
			}
		}()
	...
	for stage = 0; stage < len(twiddles); stage++ {
		...
		for nn := 0; nn < nbLoops; nn++ {
			...
			// we have to split len(b) - m by nCpus
			if (len(b)-m)%nCpus != 0 {
				panic("happens")
			}
			interval := (len(b) - m) / nCpus
			nbIntervals := (len(b) - m) / interval

			wg.Add(nbIntervals)
			for i := 0; i < nbIntervals; i++ {
				start := i * interval
				end := start + interval
				chWork <- bWork{&wg, b, m, stage, start, end}
			}
	...
	close(chWork) // kill the go routines
